...
Ext.ux.tree.TreeGrid = Ext.extend(Ext.tree.TreePanel, {
	...
	initColumns : function() {
		...
		for (i = 0; i < len; i++) {
			c = cs[i];
			if (!c.isColumn) {
				c.xtype = c.xtype ? (/^tg/.test(c.xtype) ? c.xtype : 'tg'
						+ c.xtype) : 'tgcolumn';
				c = Ext.create(c);
			}
			c.init(this);
	...
	fitColumns : function() {
		var nNewTotalWidth = this.getInnerWidth()
		...
		for (n = 0; n < cs.length; n++) {
			...
			cs[n].width = Math.floor((nNewTotalWidth / 100)
					* (cs[n].width * 100 / nOldTotalWidth))
					- 1;
			nUsed += cs[n].width;
		}
	...
	onResize : function(w, h) {
		...
		if (Ext.isNumber(h)) {
			bd.style.height = this.body.getHeight(true) - hd.offsetHeight
					+ 'px';
		}
		...
		if (Ext.isNumber(w)) {
			...
			if (this.reserveScrollOffset
				...
				setTimeout(function() {
							me
									.setScrollOffset(bd.offsetWidth
											- bd.clientWidth > 10 ? sw : 0);
						}, 10);
			}
		}
	...
	updateColumnWidths : function() {
		var cols = this.columns, colCount = cols.length, groups = this.outerCt
				.query('colgroup'), groupCount = groups.length, c, g, i, j;
		...
		Ext.fly(this.innerHd.dom.firstChild).setWidth(tcw
				+ (this.scrollOffset || 0));
	...
	handleHdDown : function(e, t) {
		...
		if (hd && Ext.fly(t).hasClass('x-grid3-hd-btn')) {
			var ms = this.hmenu.items, cs = this.columns, index = this
					.findHeaderIndex(hd), c = cs[index], sort = c.sortable;
